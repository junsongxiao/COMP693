...
class Agents:
    ...
    def add_agent(first_name, last_name, email, phone, wechat, agency_name ):
        query = """
            INSERT INTO Agents (FirstName, LastName, Email, Phone, Wechat, AgencyName)
            VALUES (%s, %s, %s, %s, %s, %s);
        """
        # Execute the query
        return database_execute_action(query, (first_name, last_name, email, phone, wechat, agency_name))
    @staticmethod
    def get_all_agents():
